...
Deep learning is a sub-field of machine learning concerned with the algorithms to make computers to learn from experience and understand the
world in terms of heirarchy of concepts which simulates the function of a brain using artifical neural networks.
...
The goal of this algorithm is to find the best possible line (linear function `Y`)
that correctly divides the differnt group of data points (`X1,..Xn`) by slightly
modifying the weights (`W1,..Wn`) and bais `b` with small incremental steps (`learning_rate`).
The number of incremental steps to take is given by `epochs`. In the below example, it took
`50` `cycles/epochs` to find the best line.

**Example**

<iframe
  style="width:100%; height:550px; border:none"
  src="https://codewaseem.github.io/PerceptronPlayground/index.html"
/>

### Non-Linear Regions

The above algorithm works good when we can divide the data points with a linear line.
But if the data was complex and a straight line could not divide the points properly, we need a
different approach.

#### Error-function

To solve this, we make use of a error-function, error-function tells is the
distance that a line is away from the misclassified points. The lesser the distance, the more
accurate we get. This is called **gradient descent**

**Properties of Error-Function**

- It should be continious
- It should be differentiable

#### Predicition

In the previous perceptron algorithm our prediction was discrete, i.e we either got `1` or `0` and nothing
in-between.

In a continious prediction we will have the probability of some possible outcome.

To change our prediction from discrete to continious, we change the **activation function** from **step-function**
to **sigmoid-function**.
...
where `x` is the score given ` w1``x1 `+` w2``x2 `+...`b`

## Multi-Class Classification

Both version of the above perceptron could only classify a point into one of the two class. But if we
have more than two classes, we use **softmax-function**.

### Softmax-Function

The **softmax-function** gives as the probability of each class is given by

```
P(class i) = e^Zi / (e^z1 + ... + e^zn);
```

Where `Zi`, is the score of each class.

## How Good a Model is?

To find how good a model is we make use of **cross-entropy** it is the negative sum of all the `ln(P(i))`.
The lower the number the better the model is.

There is also a multi-class cross-entropy.
